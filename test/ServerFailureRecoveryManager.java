...
public class ServerFailureRecoveryManager {
	...
	private static String getJavaCommand() {
		...
		if (System.getProperty("os.name").startsWith("Win")) {
			java_command = System.getProperties().getProperty("java.home") +
	...
	private static List<String> getServerStartCommand(String node) {
		List<String> cmd = getServerStartCommand();
		if (GraderCommonSetup.TEST_FAULT_TOLERANCE && GraderFaultTolerance
				.GIGAPAXOS_MODE) {
			cmd.add(node);
			cmd.add("start");
	...
	private static Process startProcess(List<String> commandArgs, String node)
			throws IOException {
		File file = new File(node + ".out");
		return GraderCommonSetup.REDIRECT_IO_TO_FILE ? processBuilder
				.redirectOutput(file).redirectError(file).directory(new File
						(System.getProperty("user.dir"))).command(commandArgs)
				.start()
	...
	private Map<String, InetSocketAddress> reconfigurators = new
	...
	protected synchronized static Process startServer(String node) throws
			IOException {

		Process p;
		// server already running
		if (serverPIDs.containsKey(node) && (p = serverPIDs.get(node))
				.isAlive())
			return p;

		// else start server
		System.out.println("Recovering " + node);
		serverPIDs.put(node, (p = startProcess(getServerStartCommand(node),
				node)));
		return p;
	}
	 ...
	 */
	protected synchronized static Process recoverServer(String node) throws
			IOException {
		return startServer(node);
	}
	...
	protected synchronized static Process restartServer(String node) throws
			IOException {
		killServer(node);
		return startServer(node);
	}
	...
	protected static Set<String> killRandomServers(int numServers) throws
			IOException {
	 ...
	 */
	protected static void killAndRecoverRandomServer(long millis) throws
			IOException, InterruptedException {
		recoverServer(killRandomServer(), millis);
	}

	// schedule server recovery after millis
	protected static void recoverServer(String server, long millis) throws
			IOException, InterruptedException {
	...
	protected static void killServer(String server, long millis) throws
			IOException, InterruptedException {
	...
	protected static void killAndRecoverRandomServers(int numServers, long
	...
	private static long perturb(long millis, double fractionalPerturbation) {
		return (long) (millis * ((1 - fractionalPerturbation) + Math.random()
				* fractionalPerturbation * 2));
	}
	 ...
	 */
	protected static Set<String> serialKill(int numServers, long
			interKillMillis) throws IOException, InterruptedException {
		Set<String> hitList = getRandomServers(numServers);
		long fate = 0;
	...
	protected static void serialRecover(Set<String> recoverList, long
			startTime, long interRecoveryTime) throws IOException,
	...
	protected static void serialKillAndThenSerialRecover(int numServers, long
			killInterval, long recoverInterval) throws IOException,
			InterruptedException {
		serialRecover(serialKill(numServers, killInterval), (long)
				(killInterval * (1 + PERTURB_FRAC) * numServers),
				recoverInterval);
	}
	...
	protected static void stopRandomFailureRecovery(long timeout) throws
			InterruptedException {
	...
	private static boolean isAlive(String server) {
		return serverPIDs.containsKey(server) && serverPIDs.get(server)
				.isAlive();
	}
	...
	public static void main(String[] args) throws IOException,
		...
		try {
			...
			System.out.println("Starting " + servers[0] + " with output/error"
					+ " being redirected to " + (GraderCommonSetup
					.REDIRECT_IO_TO_FILE ? servers[0] + ".out" :
					"stdout/stderr"));
			...
			while (p.isAlive()) ;
			System.out.println("Time since kill and isAlive=false = " + (
					(System.nanoTime() - t) / 1000.0 + "us"));
			...
			while (System.currentTimeMillis() < t + 5000) {
				System.out.println(getNumAlive() + " out of " +
						nodeConfigServer.getNodeIDs().size() + " servers " +
						"alive");
				Thread.sleep(250);
			}
			stopRandomFailureRecovery(1000);
			System.out.println(getNumAlive() + " out of " + nodeConfigServer
					.getNodeIDs().size() + " servers alive " + "at the end of " +
					"" + "" + "" + "" + "" + "" + "" + "" + "" + "" + "" + ""
					+ "" + "" + "" + "" + "" + "" + "" + "" + "" + "" + "" +
					"" + "" + "" + "" + "" + "" + "" + "" + "" + "" + "" + ""
					+ "" + "" + "" + "" + "" + "" + "" + "" + "random " + "" +
